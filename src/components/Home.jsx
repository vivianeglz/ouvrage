...
class Promo extends React.Component {
  render () {
    const chiefPresentation = this.props.chief ? (<p> Délégué : {this.props.chief.first_name} {this.props.chief.last_name} AKA {this.props.chief.username}</p>) : (<p>Pas de délégué</p>);
...
class PromoList extends React.Component {
  render () {
    const listClass = this.props.promos.map(promo => <Promo promo={promo} key={promo.id} students={this.props.students} chief={this.props.students.filter(student => student.id === promo.idChief)[0]}/>);
    return (
    <ol className="grid-3">
      {listClass}
    </ol>);
    ...
    return (
      <div>
        ...
        <section>
          ...
          <PromoList promos={this.props.promos} students={this.props.students}/>
        </section>
      </div>
    );
  }
